...
export class JewishDate /*implements Comparable<JewishDate>, Cloneable*/ {
    ...
    private static getDaysInJewishMonth(month: number, year: number): number {
        if ((month === JewishDate.IYAR) || (month === JewishDate.TAMMUZ) || (month === JewishDate.ELUL) || ((month === JewishDate.CHESHVAN) && !(JewishDate.isCheshvanLong(year)))
                || ((month === JewishDate.KISLEV) && JewishDate.isKislevShort(year)) || (month === JewishDate.TEVES)
                || ((month === JewishDate.ADAR) && !(JewishDate.isJewishLeapYear(year))) || (month === JewishDate.ADAR_II)) {
            return 29;
        } else {
            return 30;
        }
...
*/
    ...
    public compareTo(jewishDate: JewishDate): number {
        return this.gregorianAbsDate < jewishDate.getAbsDate() ? -1 : this.gregorianAbsDate > jewishDate.getAbsDate() ? 1 : 0;
    }
