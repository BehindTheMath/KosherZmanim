...
export default class SunTimesCalculator extends AstronomicalCalculator {
     ...
     */
    private static readonly DEG_PER_HOUR: number = 360.0 / 24.0;
    ...
    private static getSunRightAscensionHours(sunTrueLongitude: number): number {
        ...
        let ra: number = 360.0 / (2.0 * Math.PI) * Math.atan(a);

        const lQuadrant: number = Math.floor(sunTrueLongitude / 90.0) * 90.0;
        const raQuadrant: number = Math.floor(ra / 90.0) * 90.0;
